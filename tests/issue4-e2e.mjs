...
//
// 测试阶段：
//   1 未配置（无 config.json）      → 400 title-required / config-not-configured / invalid-json / 405 / 413
//   2 损坏 config.json              → 500 CONFIG_INVALID
//   3 无效 token                   → 401（透传 GitHub 状态码 + 可读提示）
//   4 不存在的 repo                → 404（透传 + 可读提示）
//   5 有效配置真实创建              → 201 { number, html_url }；body 省略；响应不含 token
...
await removeConfig();
{
  let r = await call('/api/issue-panel/create', { body: { title: '', body: 'x' } });
  ...
  r = await call('/api/issue-panel/create', { body: { title: '未配置测试' } });
  ...
  r = await call('/api/issue-panel/create', { raw: `{"title":"x","body":"${'a'.repeat(1024 * 1024 + 100)}"}` });
...
{
  const r = await call('/api/issue-panel/create', { body: { title: '损坏配置测试' } });
...
{
  const r = await call('/api/issue-panel/create', { body: { title: '无效token测试' } });
...
{
  const r = await call('/api/issue-panel/create', { body: { title: '404测试' } });
...
await writeConfig({ repo: REPO, token: TOKEN });
{
  ...
  let r = await call('/api/issue-panel/create', {
    body: { title: `[验收测试] #4 推送路由 ${ts}`, body: '由 tests/issue4-e2e.mjs 创建，验证后将关闭。' },
  });
  check('创建成功 → 201', r.status === 201, `got ${r.status} ${JSON.stringify(r.data)}`);
  ...
  r = await call('/api/issue-panel/create', { body: { title: `[验收测试] #4 无body ${ts}` } });
  check('无 body 字段 → 201', r.status === 201, `got ${r.status} ${JSON.stringify(r.data)}`);
  if (r.status === 201) created.push(r.data.number);
