...
// P1-03（#10 评审）：模块级 permissionWarned 标志在测试进程内跨用例残留，
// 导出测试专用重置函数保证测试隔离（避免顺序敏感）。生产代码不得调用。
/** 测试专用：重置进程内权限提醒标志。仅测试环境使用。 */
export function __TEST_ONLY_resetPermissionWarned() {
...
export function permissionWarning(mode, platform) {
  ...
  if (mode === null) {
    // P1-02（#10 评审）：文案不带「stat 失败」等技术细节——对非技术用户，
    // 「无法校验」已足够（可能是临时权限拒绝/竞态删除等边缘情况，非 bug）。
    return '无法校验 config.json 权限，建议执行 chmod 600 保护 token';
  }
