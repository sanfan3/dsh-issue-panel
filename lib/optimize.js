...
export function buildOptimizePrompt(draft) {
  ...
  return [
    ...
    '3. 引用：必须保留用户已填的全部引用条目（不得删除或改写）；如果用户在标题或任务中提到了相关的 GitHub issue 号（#数字）或 API 文档链接，可以提取补充到引用中并合并去重；不要编造不存在的 issue 号或链接（headless 无联网能力，无法查询 GitHub）。',
 ...
 * @param {unknown} raw
 * @returns {string} 规范化后的条目；空输入/纯前缀返回 ''
 */
export function normalizeAcceptItem(raw) {
  const s = typeof raw === 'string' ? raw.trim() : '';
  if (s === '') return '';
  // P2-02：前缀后必须跟有内容才视为有效条目（「- [ ] 内容」/「- [x] 内容」）；
  // 纯前缀（无 .+ 内容）按空处理。
