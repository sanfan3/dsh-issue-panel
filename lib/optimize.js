...
export function runHeadless(prompt, options = {}) {
  ...
  return new Promise((resolvePromise) => {
    ...
    let child;
    try {
      child = execFileImpl(nodePath, [dshBin, '--profile', 'headless', prompt], {
        timeout: timeoutMs,
        maxBuffer,
        env: spawnEnv,
        ...
        if (error) {
          ...
          resolvePromise({ ok: false, error: { code: 'optimize-spawn-failed', message: '无法启动 dsh 优化进程：' + (error && error.message ? error.message : String(error)) } });
          return;
        }
