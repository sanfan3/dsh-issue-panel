 ...
 * @param {object} [options] 测试注入点：execFileImpl / dshBin / nodePath / timeoutMs / maxBuffer / env / headlessHome
...
export function runHeadless(prompt, options = {}) {
  ...
  return new Promise((resolvePromise) => {
    ...
    // env 注入：DSH_HOME 默认与宿主实例一致（配置/凭据/headless profile 同源），
    // 关闭遥测（一次性优化调用无需上报，也避免拖慢退出）。
    // 可用 options.headlessHome 覆盖：当宿主 settings.yaml 的默认 provider 指向
    // headless profile 未安装的插件（如 vision-toolkit-deepseek-official，报
    // NO_ADAPTER）时，handleOptimize 会从 config.json 的 headlessHome 读一个
    // settings 为 deepseek-official 的独立 DSH_HOME（本机配置，不入公共仓库）。
    const spawnEnv = { ...env, DSH_HOME: headlessHome || resolveDshHome(env), DSH_TELEMETRY_DISABLED: '1' };
