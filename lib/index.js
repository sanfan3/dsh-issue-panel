...
function sendJson(res, status, payload) {
  ...
  // 这里吞掉并留痕，保证「任何情况下不崩溃」。
  res.on('error', (error) => console.warn('[dsh-issue-panel] response write failed (client aborted?):', error && error.message ? error.message : String(error)));
  try {
...
function handleConfigError(res, error, tag) {
  ...
  sendJson(res, 500, { error: { code: 'config-unexpected', message: '读取配置时发生未知错误' } });
}
...
/** repo 格式校验：owner/repo，仅允许 GitHub 允许的字符（防 URL 注入），单段 1~100 字符（GitHub 仓库名上限，评审 P2-02）。 */
...
async function handleCreate(req, res) {
  ...
  try {
    ...
    if (error && error.code === 'BODY_TOO_LARGE') {
      sendJson(res, 413, { error: { code: 'body-too-large', message: '请求体过大（上限 1MB）' } });
    } else {
