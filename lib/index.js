// dsh-issue-panel —— host 插件（最简版 v0.1.0 → 完整版 v1.0 演进中）
// 路由：
//   #3  GET  /api/issue-panel/config       读配置（repo/configured，永不暴露 token）
//   #4  POST /api/issue-panel/create       调 GitHub REST API 创建 issue
//   #37 POST /api/issue-panel/optimize     spawn `dsh --profile headless` 优化四字段（FR-3）
//
// 实现要点：
...
import { buildOptimizePrompt, parseOptimizeOutput, normalizeDraft, mergeRefs, runHeadless } from './optimize.js';
...
/**
 * POST /api/issue-panel/create —— 创建 GitHub issue（#4 + #40 分节拼装）。
 * 请求体：{ title: string（必填）, task?: string, refs?: string[], acceptItems?: string[]（必填 ≥1）}
 *   —— 四字段 draft（#36 client 发送；#40 起正文由 host 分节拼装，不再接受「描述即 body」）。
 * 201：{ number, html_url }。
 * 400：标题为空（title-required）/ 验收标准为空（acceptance-required）/ 请求体非法（invalid-json）/
 *      未配置（config-not-configured）/ repo 格式错误（repo-invalid）。
...
async function handleCreate(req, res) {
  ...
  try {
    ...
    sendJson(res, 400, { error: { code: 'invalid-json', message: '请求体必须是合法 JSON，如 {"title": "...", "task": "...", "refs": [], "acceptItems": ["- [ ] ..."]}' } });
    return;
  }
  ...
  // 2. 规整为四字段 draft（#38 normalizeDraft：类型容错 + acceptItems 前缀规范化，与 optimize 路由共用）
  const draft = normalizeDraft(payload);

  // 3. title 必填校验（#7 前端会本地拦截，后端仍强制校验）
  if (draft.title === '') {
    sendJson(res, 400, { error: { code: 'title-required', message: '标题是必填的' } });
    return;
  }
  // #40：验收标准至少一条（PRD FR-4「## 验收标准 永远有」；#36 前端本地拦截，后端强制校验）。
  // 空行在 normalizeDraft 已被过滤，故 acceptItems.length === 0 即「无有效验收标准」。
  if (draft.acceptItems.length === 0) {
    sendJson(res, 400, { error: { code: 'acceptance-required', message: '验收标准至少写一条' } });
    return;
  }
  // #40：正文分节拼装（引用→任务→验收标准，按有无拼接；验收标准永远有）——替换旧「描述即 body」。
  const body = buildIssueBody(draft);

  // 4. 读取配置：未配置/损坏 → 明确错误，不触发 GitHub 调用
  let config;
  try {
  ...
  // 5. 调 GitHub REST API 创建 issue（token 仅用于 Authorization 头，不进响应）
  //    统一超时覆盖 fetch 连接 + 响应体读取全段（评审 P2-03）：timer 在读到响应体之后才清除，
  //    若上游在响应体阶段挂起，AbortController 仍有效，超时即中止。
  const ghBody = { title: draft.title, body };
  let response;
  let created;
  ...
  try {
    ...
    if (error && error.name === 'AbortError') {
      // #9 评审第 2 轮 P1-02：不再在 catch 内重复 clearTimeout——finally 已统一兜底
      // 清理（正常/异常/超时路径都会执行），重复清理是冗余且注释容易误导维护者。
    ...
    // #9 评审第 2 轮 P1-02：统一在 finally 清理 timer——无论正常完成 / 异常 / 超时，
    // 都确保 setTimeout 回调不再排队执行（clearTimeout 对已清理 timer 安全，无副作用）。
    clearTimeout(timer);
  }
...
/**
 * POST /api/issue-panel/optimize —— 调 dsh headless 优化四字段（#37，FR-3）。
 * 请求体：{ title: string（必填）, task?: string, refs?: string[], acceptItems?: string[] }。
 * 成功（headless exit 0）：200 { ok: true, value: { draft: 优化四字段, parseError: string|null } }——
 *   parseError 非 null 表示 headless 输出了内容但无法解析为四字段（尽力提取为空 draft，客户端据此提示而非替换表单）；
 *   引用已合并：用户已填 ∪ AI 补充，去重不覆盖（PRD FR-3 规则 3）。
 * 失败（错误信封，均不崩溃）：
 *   405 非 POST；413 请求体超限；400 invalid-json / title-required（「标题是必填的」）；
 *   502 { ok: false, error: { code, message } } —— 超时 / headless 非零退出 / 输出超限 / 无法定位 dsh 启动器。
 * 安全：本路由不读 GitHub token；dsh headless 用 dsh 自带凭据（$DSH_HOME settings + credentials）。
 */
async function handleOptimize(req, res) {
  ...
  // 1. 读取并解析请求体（复用 #4 的 readBody：1MB 上限）
  let raw;
  try {
    raw = await readBody(req);
  } catch (error) {
    if (error && error.code === 'BODY_TOO_LARGE') {
      sendJson(res, 413, { ok: false, error: { code: 'body-too-large', message: `请求体过大（已接收 ${error.size || 0} 字节，上限 ${BODY_LIMIT} 字节）` } });
    } else {
      console.error('[dsh-issue-panel] read body failed:', error);
      sendJson(res, 400, { ok: false, error: { code: 'bad-request', message: '读取请求体失败' } });
    }
    return;
  }
  ...
  try {
    ...
    sendJson(res, 400, { ok: false, error: { code: 'invalid-json', message: '请求体必须是合法 JSON，如 {"title": "...", "task": "...", "refs": [], "acceptItems": ["- [ ] ..."]}' } });
    return;
  }
  if (payload === null || typeof payload !== 'object' || Array.isArray(payload)) {
    sendJson(res, 400, { ok: false, error: { code: 'invalid-json', message: '请求体应为 JSON 对象' } });
    return;
  }

  // 2. 规整为四字段 + title 必填（#37 验收：空 title 400「标题是必填的」；
  //    acceptItems 至少一条由 #36 客户端本地拦截，服务端不重复强制——PRD §6 仅要求推送时校验）
  const draft = normalizeDraft(payload);
  if (draft.title === '') {
    sendJson(res, 400, { ok: false, error: { code: 'title-required', message: '标题是必填的' } });
    return;
  }

  // 3. 组装提示词 → spawn dsh headless（120s 超时 / 4MB 输出，见 runHeadless）
  const prompt = buildOptimizePrompt(draft);
  // headless 的 DSH_HOME 可经 config.json 的 headlessHome 覆盖：当宿主 settings.yaml
  // 的默认 provider 指向 headless 未安装的插件（NO_ADAPTER）时，指向一个 settings 为
  // deepseek-official 的独立 DSH_HOME（如工作区影子 HOME）。配置项不入公开仓库。
  let headlessHome;
  try {
    const cfg = await loadIssuePanelConfig();
    headlessHome = cfg?.headlessHome || undefined;
  } catch {
    headlessHome = undefined; // 配置缺失/损坏不影响优化默认路径
  }
  const result = await runHeadless(prompt, { headlessHome });
  if (!result.ok) {
    // #37 验收：超时/headless 失败返回含原因的错误信封，不崩溃；错误信封带 ok:false 供客户端统一处理
    sendJson(res, 502, { ok: false, error: result.error });
    return;
  }

  // 4. 容错解析输出 + 引用合并（#38 解析逻辑；parseError 时 draft 为空，客户端据此提示不替换表单）
  const parsed = parseOptimizeOutput(result.stdout);
  const optimized = {
    ...parsed.draft,
    refs: mergeRefs(draft.refs, parsed.draft.refs),
  };
  console.log(`[dsh-issue-panel] optimize ok: title="${optimized.title}" parseError=${parsed.error === null ? 'null' : 'set'}`);
  sendJson(res, 200, { ok: true, value: { draft: optimized, parseError: parsed.error } });
}
...
export function apply(ctx) {
  ...
  // #37：优化路由（POST /api/issue-panel/optimize）
  ctx.effect(() => server.register({
    kind: 'exact',
    path: '/api/issue-panel/optimize',
    handler: handleOptimize,
  }), 'dsh-issue-panel: POST /api/issue-panel/optimize');
}
