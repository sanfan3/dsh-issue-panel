...
//   #36 表单扩展（FR-1 四字段：标题/任务/引用/验收标准 + draft 模型 + 验收标准必填校验）——已实现
//   #39 优化对比弹窗（FR-3 交互：/optimize 调用 + 左右对比 + 确认才替换 + 引用合并落表单）——本轮
  ...
  try {
    ...
    factory: (require) => {
      ...
      var STYLES = [
        ...
        // ---- #39 左右对比弹窗（FR-3 优化issue 交互）----
        // 独立于面板 overlay 的临时模态：z-index 高于面板（9999），确认/放弃后销毁。
        '.dsh-ip-cmp-overlay{position:fixed;inset:0;z-index:10000;display:flex;align-items:center;',
        'justify-content:center;background:rgba(0,0,0,.5);}',
        '.dsh-ip-cmp{box-sizing:border-box;width:min(720px,calc(100vw - 48px));max-height:calc(100vh - 96px);',
        'overflow:auto;padding:20px;border-radius:12px;',
        'background:var(--dsw-alias-bg-primary,#ffffff);color:var(--dsw-alias-label-primary,#1f2328);',
        'box-shadow:0 12px 40px rgba(0,0,0,.35);}',
        '.dsh-ip-cmp-title{margin:0 0 12px;font-size:16px;font-weight:600;}',
        '.dsh-ip-cmp-cols{display:flex;gap:16px;}',
        '.dsh-ip-cmp-col{flex:1;min-width:0;border:1px solid ',
        'var(--dsw-alias-border-default,rgba(31,35,40,.15));border-radius:8px;padding:12px;}',
        '.dsh-ip-cmp-col-head{margin:0 0 8px;font-size:13px;font-weight:600;',
        'color:var(--dsw-alias-label-secondary,#57606a);}',
        '.dsh-ip-cmp-field{margin-bottom:10px;}',
        '.dsh-ip-cmp-field-label{font-size:12px;color:var(--dsw-alias-label-secondary,#57606a);',
        'margin-bottom:2px;}',
        '.dsh-ip-cmp-field-value{font-size:13px;line-height:1.5;white-space:pre-wrap;word-break:break-all;',
        'color:var(--dsw-alias-label-primary,#1f2328);}',
        '.dsh-ip-cmp-empty{color:var(--dsw-alias-label-tertiary,rgba(31,35,40,.45));}',
        '.dsh-ip-cmp-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:14px;}'
      ].join('');
       ...
       * - 操作区：✨ 优化issue（#39 起接 host 路由 /optimize：本地校验 → 对比弹窗 → 确认替换）、
      ...
      function addListRow(container, inputClass, kind) {
        ...
        input.type = 'text';
        // P0-01（#36 评审）：类名必须同时带 .dsh-ip-row-input（样式选择器）与字段类
        // （.dsh-ip-ref / .dsh-ip-accept，draft 收集选择器）——此前只赋字段类导致
        // 真实浏览器中列表输入框完全无样式（CSS 定义在 .dsh-ip-row-input 上）。
        input.className = 'dsh-ip-row-input ' + inputClass;
      ...
      function collectDraft() {
        ...
        // P1-01（#36 评审）：列表容器缺失（外部脚本篡改 DOM 等）时与标题/任务缺失
        // 同样按「结构异常」返回 null——此前列表缺失会静默返回空数组，把用户已填的
        // 引用/验收标准当空数据丢弃且无感知（调用方均已判空防御）。
        var refsContainer = form.querySelector('.dsh-ip-refs');
        var acceptsContainer = form.querySelector('.dsh-ip-accepts');
        if (!titleInput || !taskInput || !refsContainer || !acceptsContainer) return null;
        return {
       ...
       * - P2-02（#38 评审，host/client 同规则）：纯前缀无内容（「- [ ]」「- [x]」）→ 返回 ''，
       *   避免空验收项残留（此前原样返回无法被 filter 剔除）。
       * - P1-02（#38 第 2 轮评审，host/client 同规则）：前缀后仅零宽/不可见字符
       *   （U+200B、U+FEFF 等）同样视为空条目——`.`/`\S` 会匹配零宽空格导致残留，
       *   显式剔除后再判空。
       */
      function normalizeAcceptItem(raw) {
        var s = (raw || '').trim();
        if (!s) return '';
        var m = /^-\s*\[[ xX]\]\s*([\s\S]*)$/.exec(s);
        if (m) {
          var content = (m[1] || '').replace(/[\u200B-\u200D\uFEFF\u2060\u180E]/g, '').trim();
          return content === '' ? '' : s;
        }
        return '- [ ] ' + s;
      }
      ...
      function handlePushClick() {
        ...
        // P1-03（#36 评审）：空白行（纯空格）在 collectDraft 被过滤，行数≠有效条数，
        // 提示补充「空白行不计入」，避免「看到 3 行却被拦截」的 UX 混淆。
        if (draft.acceptItems.length === 0) {
          showStatus(status, '⚠️ 验收标准至少写一条（空白行不计入）', true);
          return;
        }
        ...
        // #36/#40：payload = 四字段 draft。host 端 handleCreate 做正文分节拼装
        // （引用→任务→验收标准，按有无拼接，验收标准永远有，PRD FR-4）。
      ...
      /**
       * #39：✨ 优化issue 按钮点击 → 调 host 路由 POST /api/issue-panel/optimize。
       * 完整版 FR-3 流程（#37 host 路由 / #38 提示词与解析 / #39 对比弹窗，替换 #36 占位）：
       *   本地校验（标题必填 + 验收标准至少一条，同推送）→ 请求中禁用按钮 + 「优化中…」
       *   → 成功 200 { ok:true, value:{ draft, parseError } }：
       *       parseError 非 null（headless 输出无法解析）→ 提示且**不替换表单**（#37/#38 契约）；
       *       parseError 为 null → 弹出左右对比弹窗（左=原文、右=优化后），
       *       「放弃（不改动）」表单保持原样；「✓ 确认替换」才写入四字段（引用已由 host 合并）。
       *   → 失败（502 错误信封 / 400 校验 / 网络）→ 状态行展示可读错误，不弹窗。
       * 防重入：请求期间 disabled（与推送侧一致，双保险）。
       */
      function handleOptimizeClick() {
        var form = document.querySelector('.dsh-ip-form');
        if (!form) return;
        var status = form.querySelector('.dsh-ip-status');
        var optimizeBtn = form.querySelector('.dsh-ip-btn-optimize');
        if (!status || !optimizeBtn) {
          debugLog('[dsh-issue-panel] optimize aborted: form nodes missing', {
            status: !!status, btn: !!optimizeBtn,
          });
          return;
        }
        ...
        showStatus(status, '优化中…', false);

        fetch('/api/issue-panel/optimize', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(draft),
        })
          ...
          .then(function (result) {
            // 热重载竞态防御：请求期间插件可能被卸载（节点已移除），重新查询判空。
            var status2 = document.querySelector('.dsh-ip-status');
            var optimizeBtn2 = document.querySelector('.dsh-ip-btn-optimize');
            if (optimizeBtn2) optimizeBtn2.disabled = false;
            if (!status2) return;
            if (result.ok && result.data && result.data.ok && result.data.value && result.data.value.draft) {
              var optimized = result.data.value.draft;
              if (result.data.value.parseError) {
                // #37/#38 契约：headless 退出 0 但输出无法解析 → 提示且不替换表单（200 信封）。
                showStatus(status2, '⚠️ ' + result.data.value.parseError + '（表单未改动）', true);
                return;
              }
              // 成功 → 左右对比弹窗：确认才替换，放弃不动（PRD FR-3）。
              createCompareModal(draft, optimized, function () {
                applyOptimizedDraft(optimized);
                var status3 = document.querySelector('.dsh-ip-status');
                if (status3) showStatus(status3, '✨ 已应用优化结果', false);
              }, null);
            } else {
              var msg = (result.data && result.data.error && result.data.error.message) || '优化失败，请稍后重试';
              showStatus(status2, '⚠️ ' + msg, true);
            }
          })
          .catch(function (error) {
            ...
            var optimizeBtn2 = document.querySelector('.dsh-ip-btn-optimize');
            if (optimizeBtn2) optimizeBtn2.disabled = false;
            console.warn('[dsh-issue-panel] optimize failed:', error && error.message ? error.message : String(error));
            if (status2) showStatus(status2, '⚠️ 无法连接服务，请稍后重试', true);
          });
      }

      /**
       * #39：创建左右对比弹窗（FR-3 交互，PRD §3.3）。
       * - 独立临时模态挂 body（z-index 10000 > 面板 overlay 9999），非 P0-01 常驻 overlay
       *   （P0-01 约束仅针对表单面板 overlay，对比弹窗每次优化成功后新建、确认/放弃后销毁）；
       * - 左列 = 用户原文（original draft 四字段），右列 = dsh 优化后（optimized draft），
       *   空字段显示「（空）」（验收标准 1）；
       * - 按钮：`放弃（不改动）` → 只关闭弹窗（onAbandon 无操作，表单保持原样）；
       *   `✓ 确认替换` → 关闭弹窗并回调 onConfirm（由调用方 applyOptimizedDraft 写回表单）。
       * 防御：document 缺失（热重载清理竞态）时静默返回不崩溃。
       * @param {{title:string,task:string,refs:string[],acceptItems:string[]}} original 用户原文
       * @param {{title:string,task:string,refs:string[],acceptItems:string[]}} optimized 优化后
       * @param {function|null} onConfirm 确认替换回调（弹窗已从 DOM 移除后调用）
       * @param {function|null} onAbandon 放弃回调（可选，当前无额外动作）
       */
      function createCompareModal(original, optimized, onConfirm, onAbandon) {
        if (typeof document === 'undefined' || !document.body || !document.body.appendChild) return;
        var overlay = document.createElement('div');
        overlay.className = 'dsh-ip-cmp-overlay';
        overlay.dataset.dshIssuePanelCompare = '';
        var modal = document.createElement('div');
        modal.className = 'dsh-ip-cmp';
        modal.setAttribute('role', 'dialog');
        modal.setAttribute('aria-label', '优化对比');
        var title = document.createElement('div');
        title.className = 'dsh-ip-cmp-title';
        title.textContent = '✨ 优化对比';
        var cols = document.createElement('div');
        cols.className = 'dsh-ip-cmp-cols';
        cols.appendChild(renderCompareCol('你写的', original));
        cols.appendChild(renderCompareCol('dsh 优化后', optimized));
        var actions = document.createElement('div');
        actions.className = 'dsh-ip-cmp-actions';
        var abandonBtn = document.createElement('button');
        abandonBtn.type = 'button';
        abandonBtn.className = 'dsh-ip-btn dsh-ip-btn-ghost';
        abandonBtn.textContent = '放弃（不改动）';
        abandonBtn.setAttribute('aria-label', '放弃（不改动）');
        abandonBtn.addEventListener('click', function () {
          overlay.remove();
          if (typeof onAbandon === 'function') onAbandon();
        });
        var confirmBtn = document.createElement('button');
        confirmBtn.type = 'button';
        confirmBtn.className = 'dsh-ip-btn dsh-ip-btn-primary';
        confirmBtn.textContent = '✓ 确认替换';
        confirmBtn.setAttribute('aria-label', '确认替换');
      ...
      /**
       * #39：渲染对比弹窗的一列（左=你写的 / 右=dsh 优化后），四字段逐项展示。
       * 空字段（字符串空白或空数组）显示「（空）」（验收标准 1）。
       * @param {string} heading 列标题（「你写的」/「dsh 优化后」）
       * @param {{title:string,task:string,refs:string[],acceptItems:string[]}} draft
       */
      function renderCompareCol(heading, draft) {
        var col = document.createElement('div');
        col.className = 'dsh-ip-cmp-col';
        var head = document.createElement('div');
        head.className = 'dsh-ip-cmp-col-head';
        head.textContent = heading;
        col.appendChild(head);
        var d = draft && typeof draft === 'object' ? draft : {};
        col.appendChild(renderCompareField('标题', typeof d.title === 'string' ? d.title : ''));
        col.appendChild(renderCompareField('任务', typeof d.task === 'string' ? d.task : ''));
        col.appendChild(renderCompareField('引用', Array.isArray(d.refs) ? d.refs.join('\n') : ''));
        col.appendChild(renderCompareField('验收标准', Array.isArray(d.acceptItems) ? d.acceptItems.join('\n') : ''));
        return col;
      }

      /**
       * #39：渲染单个字段块（标签 + 值）。空值显示「（空）」并加 .dsh-ip-cmp-empty 样式。
       * @param {string} label 字段名（标题/任务/引用/验收标准）
       * @param {string} value 字段值（已 join 或原文）
       */
      function renderCompareField(label, value) {
        var field = document.createElement('div');
        field.className = 'dsh-ip-cmp-field';
        var lbl = document.createElement('div');
        lbl.className = 'dsh-ip-cmp-field-label';
        lbl.textContent = label;
        var val = document.createElement('div');
        val.className = 'dsh-ip-cmp-field-value';
        var v = (typeof value === 'string' ? value : '').trim();
        if (v === '') {
          val.textContent = '（空）';
          val.className += ' dsh-ip-cmp-empty';
        } else {
          val.textContent = v;
        }
      ...
      /**
       * #39：确认替换——把优化后四字段写入表单。
       * - 标题/任务直接赋值（任务 autoResize 复位高度）；
       * - 引用/验收标准列表重建为优化后条目（fillList：清空行 → 逐条填充；
       *   引用已是 host 端 mergeRefs 合并结果 = 用户已填 ∪ AI 补充，去重不覆盖，PRD FR-3 规则 3；
       *   空列表留单行空输入，可继续编辑）；
       * - 热重载竞态防御：form 节点缺失时静默返回。
       * @param {{title:string,task:string,refs:string[],acceptItems:string[]}} optimized
       */
      function applyOptimizedDraft(optimized) {
        var form = document.querySelector('.dsh-ip-form');
        if (!form) return;
        var titleInput = form.querySelector('.dsh-ip-title');
        var taskInput = form.querySelector('.dsh-ip-task');
        var opt = optimized && typeof optimized === 'object' ? optimized : {};
        if (titleInput) titleInput.value = typeof opt.title === 'string' ? opt.title : '';
        if (taskInput) {
          taskInput.value = typeof opt.task === 'string' ? opt.task : '';
          autoResize(taskInput);
        }
        fillList(form, '.dsh-ip-refs', 'dsh-ip-ref', 'ref', Array.isArray(opt.refs) ? opt.refs : []);
        fillList(form, '.dsh-ip-accepts', 'dsh-ip-accept', 'accept', Array.isArray(opt.acceptItems) ? opt.acceptItems : []);
      }

      /**
       * #39：清空列表容器行并按 values 逐条重建（确认替换用）。
       * - 只删本插件创建的 .dsh-ip-row（同 resetList 的 P1-02 约束，不动外部节点）；
       * - values 为空 → 补一行空输入（保持列表可继续编辑，不破坏「至少一条」由校验兜底）；
       * - 每行 value 直接赋值（引用/验收标准无需再规范化——host 端已规范化）。
       */
      function fillList(form, containerSel, inputClass, kind, values) {
        var container = form.querySelector(containerSel);
        if (!container) return;
        removeListRows(container);
        if (!values || values.length === 0) {
          addListRow(container, inputClass, kind);
          return;
        }
        for (var j = 0; j < values.length; j++) {
          var row = addListRow(container, inputClass, kind);
          var input = row.querySelector('input');
          if (input) input.value = String(values[j]);
        }
      }
       ...
       * 引用/验收标准列表重置为初始单行空输入。
       * P1-04（#36 评审）：本函数**故意不清空状态行**——状态行留存是 #8 的既定行为
       * （「✓ 已创建 issue #N」在关面板再开仍可见），由 renderSuccess 先行写入、
       * clearForm 保留；若未来重构让 clearForm 接管状态行，需同步保证成功信息不丢失。
      ...
      /**
       * #36/#39 共用：删除列表容器内的全部本插件行节点（.dsh-ip-row）。
       * P1-02（#36 评审）：仅删除本插件创建的 .dsh-ip-row，防止误删外部注入节点。
       * resetList（清空重置）与 fillList（按值重建）复用本函数，避免删行逻辑重复（DRY）。
       */
      function removeListRows(container) {
        var rows = container.querySelectorAll('.dsh-ip-row');
        for (var i = rows.length - 1; i >= 0; i--) rows[i].remove();
      }

      /**
       * #36：列表容器重置为初始状态（清空全部行 + 补一行空输入）。
       * P1-02（#36 评审）：仅删除本插件创建的 .dsh-ip-row 节点（按类名筛选），
       * 防止误删外部注入节点（浏览器插件/用户脚本向容器注入内容时，无差别清空
       * 子节点会破坏外部状态）。stub 与真实 DOM 均支持 querySelectorAll + remove。
       */
