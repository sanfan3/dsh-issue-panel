...
if (!window.__ModuleLoader__) {
  ...
  try {
    ...
    // P2-03（#10 评审）：错误文案来自 @deepseek-ai/dsh-client-modules 内部实现，
    // 字符串匹配用宽松的 'duplicate' 前缀（而非完整文案），降低上游改文案导致
    // 防御失效的概率；若上游重构错误码为 error.code，请改用稳定契约匹配。
    if (error && error.message && error.message.indexOf('duplicate') !== -1) {
